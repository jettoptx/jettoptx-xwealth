/**
 * LT (Luby transform) fountain code — Decimen-compatible.
 * Reference: bashalarmistalt/decimen-optical-transfer shared/fountain.ts
 *
 * Uses deterministic dlog() so V8 (web sender) and JSC (iOS Mojo) stay in sync.
 */
...
export class LTEncoder {
  ...
  constructor(
    ...
    for (let b = 0; b < this.k; b++) {
      const src = payload.subarray(
        b * blockLen,
        Math.min((b + 1) * blockLen, payload.length),
      );
...
/** Expected frames ≈ K × this (robust-soliton overhead) — for progress UX. */
