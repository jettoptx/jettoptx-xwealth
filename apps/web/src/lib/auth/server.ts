...
import { extractProfileImageUrl } from "./profile-image";
import {
  GROK_ISSUER_DEFAULT,
...
/** Normalize broker/OIDC userinfo into Better Auth fields (esp. avatar). */
function pickString(profile: Record<string, unknown>, keys: string[]): string | undefined {
  for (const k of keys) {
    const v = profile[k];
...
async function mapGrokProfileToUser(profile: Record<string, unknown>) {
  const image =
    extractProfileImageUrl(profile) ??
    (typeof profile.image === "string" ? profile.image : null) ??
    (typeof profile.picture === "string" ? profile.picture : null);
  // Keep human display name separate from @handle when both exist
  const name =
    pickString(profile, ["name", "display_name", "displayName"]) ||
    pickString(profile, ["preferred_username", "username", "screen_name"]) ||
    undefined;
  const username =
    pickString(profile, [
      "preferred_username",
  ...
  return {
    ...
    // additional field — X @handle when broker provides it
    ...(username ? { username } : {}),
  };
}

async function fetchGrokUserInfo(tokens: {
  ...
  try {
    ...
    const mapped = await mapGrokProfileToUser(data);
    const id =
    ...
    return {
      ...
      ...data,
      ...mapped,
      email: mapped.email ?? (typeof data.email === "string" ? data.email : undefined),
      name: mapped.name,
      image: mapped.image,
...
const grokOAuthPlugin = authConfigured
  ? genericOAuth({
      config: GROK_PROVIDERS.map(({ providerId, idp }) => ({
        ...
        // Pull picture / profile_image_url from broker userinfo into user.image
        getUserInfo: fetchGrokUserInfo,
        mapProfileToUser: mapGrokProfileToUser,
      })),
    })
  : null;
...
export const auth = betterAuth({
  ...
  account: {
    ...
    accountLinking: {
      ...
      // Refresh name/image from broker on each sign-in (avatar updates).
