/**
 * Particle Reveal — Canvas UI
 * @see https://canvasui.dev/docs/components/particle-reveal
 *
 * Content renders as dust; cursor proximity re-materializes full UI.
 * Falls back to crisp HTML when html-in-canvas / WebGL2 is unavailable.
 */
"use client";
...
export interface ParticleRevealOptions {
  ...
  size?: number;
  /** How far grains wander from their home pixel in CSS pixels. */
  scatter?: number;
  /** Speed of the idle grain shimmer (0 freezes the dust). */
  drift?: number;
  /** Chromatic aberration strength at the reveal edge in CSS pixels. */
  aberration?: number;
  /** How strongly unrevealed content smears around the reveal edge. */
  bend?: number;
  /** How strongly dust specks stand out from the background (0 to 1). */
  fade?: number;
  /** Contrast threshold for UI pixels vs empty space. */
  threshold?: number;
  /** Backdrop color for contrast detection. */
  background?: string;
  /** Seconds the reveal takes to catch up with the cursor. */
  smoothing?: number;
}
