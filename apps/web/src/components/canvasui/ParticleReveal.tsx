...
export interface ParticleRevealProps extends ParticleRevealOptions {
  ...
  /**
   * Content box overflow. Default "hidden" so compact marks (footer logos)
   * never grow scrollbars; use "auto" only for tall reveal panels.
   */
...
/** Tiny procedural noise tile for the universal (non html-in-canvas) dust cover. */
function makeNoiseDataUrl(size = 64): string {
  ...
  for (let i = 0; i < img.data.length; i += 4) {
    ...
    const speck = Math.random() > 0.72 ? n : (n * 0.35) | 0;
    img.data[i] = speck;
    img.data[i + 1] = speck;
    img.data[i + 2] = speck;
    img.data[i + 3] = Math.random() > 0.55 ? 210 : 0;
  }
...
/**
 * CSS dust reveal used when Chrome html-in-canvas is unavailable.
 * Content sits under a noise cover; cursor punches a soft hole so the UI
 * "emerges from dust" on every browser (matches Canvas UI caption UX).
 */
function FallbackDustReveal({
  children,
  background,
  radius = 500,
  softness = 0.75,
  ...
  useEffect(() => {
    setNoiseUrl(makeNoiseDataUrl(72));
  }, []);
  ...
  const inner = Math.max(8, radius * (1 - soft * 0.85));
  const outer = Math.max(inner + 24, radius);
  const px = `${(pointer.x * 100).toFixed(2)}%`;
  const py = `${(pointer.y * 100).toFixed(2)}%`;
  // Transparent at cursor (dust gone → content shows); solid elsewhere (dust covers).
  const mask = pointer.active
    ? `radial-gradient(circle ${outer}px at ${px} ${py}, transparent 0px, transparent ${inner}px, rgba(0,0,0,0.55) ${inner + (outer - inner) * 0.45}px, black ${outer}px)`
    : `radial-gradient(circle 140px at 50% 42%, transparent 0px, transparent 36px, rgba(0,0,0,0.4) 90px, black 140px)`;
  ...
  return (
    ...
    >
      ...
      {/* Dust cover — pointer-events none so links/text remain interactive */}
      <div
        aria-hidden
        className="particle-reveal-dust"
        style={{
          position: "absolute",
          inset: 0,
          pointerEvents: "none",
          ...
          backgroundImage: noiseUrl
            ? `url(${noiseUrl}), radial-gradient(ellipse 80% 70% at 50% 45%, transparent 0%, ${background} 100%)`
            : undefined,
          backgroundSize: noiseUrl ? "72px 72px, 100% 100%" : undefined,
          backgroundRepeat: noiseUrl ? "repeat, no-repeat" : undefined,
          opacity: 0.92,
          WebkitMaskImage: mask,
          maskImage: mask,
          transition: pointer.active
            ? "opacity 120ms ease-out"
            : "mask-image 400ms ease, -webkit-mask-image 400ms ease, opacity 300ms ease",
        }}
      />
    </div>
  );
...
export function ParticleReveal({
  ...
  const bg = options.background ?? initialOptions.background ?? DEFAULTS.background;
  const radius = options.radius ?? initialOptions.radius ?? DEFAULTS.radius;
  const softness = options.softness ?? initialOptions.softness ?? DEFAULTS.softness;
