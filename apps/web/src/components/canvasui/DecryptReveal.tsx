...
const CIPHER_CHARS =
  "01<>{}[]/\\|@#$%&*+=~^ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";

/**
 * Universal cipher cover when Chrome html-in-canvas is unavailable.
 * Full-bleed canvas glyphs cover the entire card; cursor punches a soft hole
 * so UI decrypts under the pointer.
 */
function FallbackCipherReveal({
  children,
  background,
  color = "#ff6900",
  radius = 400,
  ...
  const pointerRef = useRef({ x: -1e5, y: -1e5, active: 0 });
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Full card solid plate
      ctx.fillStyle = background;
      ctx.fillRect(0, 0, w, h);

      const cols = Math.ceil(w / cell) + 1;
      const rows = Math.ceil(h / cell) + 1;
      ctx.font = `600 ${cell - 1}px ui-monospace, "Space Mono", Menlo, Consolas, monospace`;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";

      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          // Scramble: stable-ish hash + time so whole field mutates
          const n =
            ((col * 73856093) ^ (row * 19349663) ^ (t * 83492791)) >>> 0;
          const ch = CIPHER_CHARS[n % CIPHER_CHARS.length];
          const x = col * cell + cell * 0.5;
          const y = row * cell + cell * 0.5;
          // Slight density variation so it reads as cipher, not solid fill
          const dim = (n >>> 8) % 5 === 0;
      ...
      // Decrypt hole: cut out cipher so real UI shows through
      const p = pointerRef.current;
      if (p.active > 0.02) {
        const soft = Math.min(1, Math.max(0.05, softness));
        const outer = Math.max(radius * 0.55, 80) * (0.65 + p.active * 0.35);
        const inner = outer * (1 - soft * 0.7);
        const g = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, outer);
        g.addColorStop(0, "rgba(0,0,0,1)");
        g.addColorStop(Math.min(0.55, inner / outer), "rgba(0,0,0,1)");
  ...
  }, [background, color, radius, softness]);
...
export function DecryptReveal({
  ...
  const bg =
    options.background ?? initialOptions.background ?? DEFAULTS.background;
  const color = options.color ?? initialOptions.color ?? DEFAULTS.color;
  const radius = options.radius ?? initialOptions.radius ?? DEFAULTS.radius;
  const softness =
    options.softness ?? initialOptions.softness ?? DEFAULTS.softness;
