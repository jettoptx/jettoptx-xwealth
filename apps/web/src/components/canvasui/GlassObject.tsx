...
export function GlassObject({
  ...
  useEffect(() => {
    ...
    // Ensure the canvas has real layout size before WebGL boots
    // (absolute fill can report 0x0 if parent height is only from content).
    const applyHostSize = () => {
    ...
    const instance = createGlassObject({ canvas }, opts);
    instanceRef.current = instance;
    if (!instance) {
      setStatus("error");
      setErrorMsg("WebGL unavailable");
    } else {
      // Nudge resize after first paint
  ...
  return (
    <div
      ref={hostRef}
      className={className}
      style={{ position: "relative", minHeight: 280, ...style }}
    >
      <canvas
        ref={canvasRef}
        style={{
      ...
      {status === "error" && (
        ...
        >
          Glass failed: {errorMsg ?? "unknown error"}
        </div>
      )}
    </div>
  );
